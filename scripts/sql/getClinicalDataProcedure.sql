CREATE PROCEDURE `getClinicalDataProcedure`(
...
BEGIN
    ...
    SET @common_select = 'SELECT DISTINCT a.id, a.name, a.gender, a.age, a.mobileNo, a.hospitalId AS hospitalId, a.registraionId as registrationId,a.createdDate FROM ';

    -- Common conditions with placeholders
    SET @common_conditions = '  c.referral = TRUE
                               AND a.optionalId <> 2 
    ...
    IF roleId = 2 THEN
        -- Program Coordinator
        SET @final_query = CONCAT(@common_select, 'candidate a left join candidatetest c on c.candidateId=a.id WHERE ', @common_conditions);

    ELSEIF roleId = 3 THEN
        -- Camp Creator
        SET @final_query = CONCAT(@common_select, 
                                  'camp b LEFT JOIN candidate a ON b.id = a.campId left join candidatetest c on c.candidateId=a.id WHERE b.createdBy = "', 
    ...
    ELSE
        ...
        SET @final_query = CONCAT(@common_select, 
                                  ...
                                  left join candidatetest c on c.candidateId=a.id 
